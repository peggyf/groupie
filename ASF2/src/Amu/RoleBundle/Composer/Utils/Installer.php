...
namespace Amu\RoleBundle\Composer\Utils;
            ...
            foreach ($arSourcesDestinationsFiles as $oneSrc=>$oneDest) {
                ...
                if($isPresent){
                    if($toCopy){
                        ...
                        $fs->copy(__DIR__ . '/../RoleBundle/' . $oneSrc, $appDir . '/Resources/' . $oneDest, true);
                        $event->getIO()->write(" => app/Resources/$oneDest => OK (écrasé)");
                    }else{
                        if($copyNew){
                            $fs->copy(__DIR__ . '/../RoleBundle/' . $oneSrc, $appDir . '/config/' . $oneDest.".new", true);
